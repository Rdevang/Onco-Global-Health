...
end_session() { sf agent preview end --api-name "$AGENT" --session-id "$1" >/dev/null 2>&1 || true; }
send() {
  ...
  echo "=== $label ==="; echo "Q: $utt"
  python3 - <<PY
import json
try:
    d=json.load(open("$outf")); msgs=d.get('result',{}).get('messages',[])
    print("A: " + (msgs[-1].get('message','(no message)') if msgs else json.dumps(d)[:400]))
...
for item in "${A[@]}"; do label="${item%%|*}"; utt="${item#*|}"; sid=$(start_session); send "$sid" "$utt" "$label"; end_session "$sid"; done
...
for item in "${C[@]}"; do label="${item%%|*}"; utt="${item#*|}"; sid=$(start_session); send "$sid" "$utt" "$label"; end_session "$sid"; done
...
for item in "${E[@]}"; do label="${item%%|*}"; utt="${item#*|}"; sid=$(start_session); send "$sid" "$utt" "$label"; end_session "$sid"; done
